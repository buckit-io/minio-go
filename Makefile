...
lint:
	@mkdir -p ${GOPATH}/bin
	@echo "Installing golangci-lint $(GOLANGCI_LINT_VERSION)" && curl -sSfL https://raw.githubusercontent.com/golangci/golangci-lint/$(GOLANGCI_LINT_VERSION)/install.sh | sh -s -- -b $(GOPATH)/bin $(GOLANGCI_LINT_VERSION)
	@echo "Running $@ check"
